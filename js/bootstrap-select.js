...
(function ($) {
  ...
  Selectpicker.DEFAULTS = {
    ...
    /* options for add if not found */
    addLabel: '<strong>Add:</strong> {0}',
    addCallback: null,
    addNew: false
  };
      ...
      this.$searchbox.on('input propertychange', function () {
        if (that.$searchbox.val()) {
          ...
          if (!that.$lis.not('.hidden, .no-results').length) {
            ...
            /**
              Not found -> Add Searched text?
            **/
            if (that.options.addNew) {
              $no_results.html('<a><span class="text">'+that.options.addLabel.replace('{0}', '"' + htmlEscape(that.$searchbox.val()) + '"')+'</span></a>').show();
              $no_results.addClass('active');
              $no_results.on('click', function() {
                var val = htmlEscape(that.$searchbox.val());
                if (that.options.addCallback) {
                  var fn = window[that.options.addCallback];
